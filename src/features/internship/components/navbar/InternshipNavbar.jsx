...
const InternshipNavbar = ({ onNavigation, isFixed = true }) => {
  ...
  return (
    <div>
          ...
          <div className="flex items-center">
            ...
            <ul className={getNavUlClassName()}>
              {navbarLinks.map((row, index) => (
                <li key={row.id} className="transition-all">
                  <a
                    href="#"
                    onClick={() => handleNavigation(row.id)}
                    className={`relative w-fit block after:block after:content-[''] ${
                      isNavbarScroll ? "after:bg-[#332C2B]" : "after:bg-white "
                    } after:absolute after:h-[2px] after:w-full after:scale-x-0 after:hover:scale-x-100 after:transition after:duration-300 after:origin-center`}
                  >
                    {row.label}
                  </a>
                </li>
      ...
      {isNavbarOpened && (
        <div className="z-10 bg-black fixed top-0 left-0 right-0 bg-opacity-80 bottom-0">
          <nav className="transition duration-400 text-center m-auto left-0 right-0 z-10 bg-white z-20">
            ...
            <ul className="text-left container mx-auto">
              ...
              <li className="w-full" style={{ transition: "all 0.3s" }}>
                <button
                  onClick={() => {

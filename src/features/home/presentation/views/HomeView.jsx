...
export default function HomeView() {
  ...
  const PrimaryButton = ({ children, className = "" }) => (
    <button
      className={`bg-[#332C2B] flex gap-x-3 mt-2 text-white font-bold text-center text-[inter] text-base rounded-full px-12 py-4 hover:opacity-90 transition duration-800 ${className}`}
    >
      {children}
    </button>
  );

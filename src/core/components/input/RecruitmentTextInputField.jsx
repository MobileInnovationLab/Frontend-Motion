...
const RecruitmentTextInputField = ({ label, name, className, containerClass, placeholder, suffix, required = false, error, ...props }) => {
  return (
    <div className={getCustomClassNameUtils("flex flex-col", containerClass)}>
      ...
      <div className="flex relative">
        ...
        {suffix && <span className="absolute right-5 top-[1.05rem] text-left">{suffix}</span>}
      </div>
    </div>
  );
};
